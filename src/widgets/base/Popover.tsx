...
var { height: SCREEN_HEIGHT, width: SCREEN_WIDTH } = Dimensions.get("screen");
...
class Popover extends React.Component<any, any> {
  ...
  computeGeometry = ({ contentSize, placement }: any) => {
  ...
  _startDefaultAnimation = ({ show, doneCallback }) => {
    ...
    var commonConfig = {
      ...
      easing: show ? Easing.out(Easing.ease) : Easing.inOut(Easing.quad)
    };
  ...
  render() {
    ...
    return (<TouchableWithoutFeedback onPress={this.props.onClose}>
      <View
        style={[
          styles.container,
